...
const InfeasibleOptionSchema = new Schema({ title: String, label: String, checked: Boolean }, { _id: false });
const ValueListsUsedSchema = new Schema({
    scenarioMoralValueReorderedName: { type: String },
    scenarioMoralValueReordered: [{ type: String, default: [] }],
    finalTopTwoValues: [{ type: String, default: [] }],
    infeasibleOptions: { type: [InfeasibleOptionSchema], default: [] },
}, { _id: false });
...
const DebugRowSchema = new Schema({
    ...
    alignmentStatus: { type: String, default: "Unknown" },
